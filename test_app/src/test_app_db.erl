...
init_sqlite() ->
    {ok, Cfg} = application:get_env(test_app, sqlite),
    Path = proplists:get_value(path, Cfg, "priv/db/test_app.db"),
    ok   = filelib:ensure_dir(Path),
    {ok, Db} = esqlite3:open(Path),
    persistent_term:put({test_app_db, conn}, Db),
    run_migrations(Db),
    ok.

conn() -> persistent_term:get({test_app_db, conn}).
...
backend() ->
    application:get_env(test_app, db_backend, sqlite).
