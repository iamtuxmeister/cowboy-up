...
init([]) ->
    ...
    ok = start_fs(test_app_fs_src,  SrcDir),
    ok = start_fs(test_app_fs_priv, PrivDir),
    fs:subscribe(test_app_fs_src),
    fs:subscribe(test_app_fs_priv),
...
terminate(_,_)     ->
    (catch fs:stop(test_app_fs_src)),
    (catch fs:stop(test_app_fs_priv)),
    ok.
...
reload_erl(Path) ->
    ...
    EbinDir = code:lib_dir(test_app, ebin),
...
reload_dtl(Path) ->
    ...
    case test_app_templates:compile_all() of
